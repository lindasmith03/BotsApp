const data = {
    ...
    all: {
        DESCRIPTION: "Module to tag everyone in a group.",
        EXTENDED_DESCRIPTION:
            "```Use this module to tag everyone in the group by either replying to a message or simply using```  *.all*  ```command.```",
    ...
    alive: {
        ...
        ALIVE_MSG: "```💠 Dobby has been integrated successfully.```"
    },
    block: {
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION: "```Get the profile picture of the group in a group conversation or the profile picture of Dobby itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by Dobby.```",
    ...
    help: {
        ...
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "🌀 *My Command Interface* 🌀\n\n",
    ...
    tehelka: {
        DESCRIPTION: "Module to tag everyone to play BGMI.",
        EXTENDED_DESCRIPTION:
            "```Use this module to tag everyone in the group to play BGMI```  *.tehelka*  ```command.```",
        TAG_MESSAGE: "*Ajao BGMI*"
    },
    tr: {
